...
export default function QuickInfo({ pkg }) {
  const items = [
    ...
    },
    {
      icon: FiCalendar,
      title: "Preparation",
      value: pkg?.fasting ? "8–10 hrs Fast" : "No Fasting",
      color: "amber",
    },
    {
      icon: FiShield,
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6">
        ...
        <div className="mx-auto mb-10 max-w-2xl text-center">
          ...
          <h2 className="mt-4 text-3xl font-black text-slate-900 sm:text-4xl">
            Everything Before
            <span className="block text-blue-600">You Book</span>
          </h2>
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 sm:gap-5 xl:grid-cols-6">
          {items.map((item) => {
            ...
            return (
              <div
                key={item.title}
                className="group rounded-2xl border border-slate-200 bg-white p-4 transition-all duration-300 hover:-translate-y-1 hover:shadow-lg sm:rounded-3xl sm:p-6"
              >
                <div
                  className={`flex h-12 w-12 items-center justify-center rounded-xl ring-8 ring-transparent transition-all ${color.bg} ${color.ring} sm:h-14 sm:w-14`}
                >
                ...
                <h3 className="mt-2 text-sm font-bold leading-6 text-slate-900 sm:text-lg">
                  {item.value}
                </h3>
              </div>
            );
