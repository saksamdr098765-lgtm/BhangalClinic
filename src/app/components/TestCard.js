...
export default function TestCard({ test, location = "Home" }) {
...
*Test:* ${test.name}
*Description:* ${test.desc || test.description}
*Price:* ${test.price}
        ...
        <div className="mt-auto pt-6">
          {test.status === "published" ? (
            ...
            <TrackedWhatsappLink
              ...
              tracking={`${location}-${test.name}`}
              aria-label={`Book ${test.name} test via WhatsApp`}
              className={`
                w-full
                h-11
                rounded-2xl
